...
| `librechat_balance_credits_by_email_domain`             | gauge | `email_domain`                       | Current balance in raw tokenCredits summed per user email domain (1e6 tokenCredits = 1 USD). Always-on (not gated).                     |
...
| `librechat_transaction_token_sum_by_user`               | gauge | `id`                                 | Sum of tokens per user across all models/tokenTypes (one series per user). Low-cardinality companion to `_by_model_user` — prefer this one for per-user totals/rankings at high user counts. Same gate and `id`-label semantics. |
| `librechat_balance_credits_by_user`                     | gauge | `id`                                 | Current balance in raw tokenCredits per user. Same gate and same `id`-label semantics. Users without a balance record emit no series.   |
...
| `librechat_agent_usage_by_user_count`           | gauge | `agent`, `id`           | Gated by `EMIT_PER_USER_METRICS=true`. `id` is the user's Mongo `_id` by default, or email when `ANONYMIZE_EMAIL_LABEL=false`, or pseudonymous HMAC-SHA256 hex when `METRICS_USER_ID_SALT` is set. |
